...
#        self.allcases = ['u3.0','u4.0']
        self.allcases = ['u4.0']
        # self.nst,self.nls,self.nin = 100100,100300,100
        self.nst,self.nls,self.nin = 100100,100100,100
        ...
        self.kst,self.kls,self.kint = 1,2,1  # only k=2 plane
        self.ist,self.ils = 0,5 # default 0:rho, 1-3:uvw, 4:p
        self.iz = 3
        self.orgpath='/share/vol5/mimura/From_AFI/rod-valid-test/work/'
    ...
    def run(self):
        ...
        if rank == root:
            ...
            while True:
                ...
                workname = 'work'+casename[len(self.casesub):]
                ...
                if casename == 'STOP':
                    ...
                    fname=self.datapath+casename+'/grid/grid.'+'{:0=5}'.format(self.iz)
                    # Check header
                    ...
                    rall,idum = readgrid(fname,iheader,8)
                    ...
                    for k,nstep in enumerate(self.nstepall):
                        fname = flowpath+'/flow_z'+'{:0=5}'.format(self.iz)+'_'+'{:0=8}'.format(nstep)
                        if np.mod(k,1) == 0:
                        ...
                        fname = 'flow_z{0:0=5}_{1:0=8}'.format(self.iz,nstep)
